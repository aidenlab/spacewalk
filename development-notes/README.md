...
| [refactor-continuous-genomic-locator.md](refactor-continuous-genomic-locator.md) | Proposal | RFC follow-on to the highlighting redesign — make the ribbon bead a *continuous* locator (glide along the curve) while the highlight stays quantized to the discrete window. The conceit: navigation is continuous, the data is discrete, the index is a projection of the continuous coordinate |
